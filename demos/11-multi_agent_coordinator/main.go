...
// Package demonstrates multi-agent coordination with safety settings.
package main
...
func main() {
	...
	rollAgent, err := llmagent.New(llmagent.Config{
		Name:        "roll_agent",
		Model:       model,
		Description: "Specialist agent for rolling dice.",
		Instruction: "You specialize in rolling dice. Help the user roll dice with various numbers of sides.",
		Tools: []tool.Tool{
			dieTool,
		},
	})
	...
	primeAgent, err := llmagent.New(llmagent.Config{
		Name:        "prime_agent",
		Model:       model,
		Description: "Specialist agent for checking prime numbers.",
		Instruction: "You specialize in identifying prime numbers. Help the user check if numbers are prime.",
		Tools: []tool.Tool{
			primeTool,
		},
	})
	...
	coordinator, err := llmagent.New(llmagent.Config{
		...
		Instruction: "You are a coordinator. Delegate dice-rolling questions to the roll_agent and prime-checking questions to the prime_agent.",
